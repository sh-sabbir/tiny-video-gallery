	...
	var YtConst = function (requestObj) {
		try {
			if (!requestObj) {
				...
				this.iframeWrap = "#youtubePlayerOverlay .videoPlayerWrap";
				...
				this._isDoneDone = {
					...
					progressMessage:
						"Your request has been accepted and processed.",
				};
				this._isBeingDone = {
	...
	cpo.createPlayerContainer = function () {
		...
		if (jQuery($this.overlayContainer).length === 0) {
			var o = jQuery(
				...
				wrap = jQuery('<div class="videoPlayerWrap"></div>'),
				cButton = jQuery(
					'<button id="youtubeOverlayCloser" class="defaultButton closeIcon"><img src="https://s3.ap-south-1.amazonaws.com/choosemybicycle/cmb-website-icons/cancel-close-icons/x-mark-black-icon.svg" class="img-responsive center-block" alt="Close" title="close"/></button>'
				),
				b = jQuery("body");

        o.appendTo(b);
        wrap.appendTo(o);
			cButton.appendTo(wrap);
		}
		$this.triggerCheck();
	};
	...
	cpo.activateOnClickHandler = function () {
		...
		jQuery(t).on("click", function () {
			...
			if (idEquals) {
				...
				jQuery(w).find("iframe").remove();
				var requiredIframe = jQuery(
				...
				requiredIframe.appendTo(jQuery(w));
	...
	cpo.closeOnClickingOverlay = function () {
		...
		overlay.on("click", function (event) {
			if (
				...
				event.target.id === "youtubePlayerOverlay"
			) {
